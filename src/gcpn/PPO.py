...
class ActorCriticGCPN(nn.Module):
    ...
    def act(self, state, memory):
        ...
        if self.crem:
            ...
            action, prob, states, batch_rewards = self.actor(state)
            memory.states.append(states)
            memory.batch_rewards.append(batch_rewards)
        else:
    ...
    def evaluate(self, state, action, batch_rewards):
        # X_agg will be (n_step, hidden_dim). Probs is (n_step,4). If self.crem X_agg is same size, probs is (2000, 1).
        probs, X_agg = self.actor.evaluate(state, action, batch_rewards)
...
class PPO_GCPN:
    ...
    def update(self, memory, i_episode, writer=None):
        ...
        rewards = []
        discounted_reward = 0
        # TODO: Examine if any is_terminals are 0, I think there's some redundancy here.
        ...
        if self.crem:
            ...
            #old_batch_rewards = [torch.tensor(batch).to(self.device).detach() for batch in memory.batch_rewards]
        ...
        for i in range(self.K_epochs):
            ...
            logprobs, state_values, entropies = self.policy.evaluate(old_states, old_actions, memory.batch_rewards)
